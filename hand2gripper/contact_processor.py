...
class ContactProcessor:
    ...
    def _process_single_sample(self, sample_id: int, color_image: Union[str, np.ndarray], depth_image: Union[str, np.ndarray]) -> np.ndarray:
        ...
        bboxes = self._read_bbox(sample_id)         # [array([592,  57, 803, 383], dtype=int32)]
        bboxes = self._xyxy_to_xywh(bboxes)         # [array([592,  57, 211, 326], dtype=int32)]
        is_right = self._read_is_right(sample_id)   # [array(False)]
        img_size = self._read_img_size(sample_id)   # array([1080, 1080], dtype=int32)
    ...
    def _save_contact_results(self, sample_id: int, hand_id: int, contact_results: dict, save_dir: str):
        os.makedirs(save_dir, exist_ok=True)
        data = {
            'contact_out': _to_numpy(contact_results['raw_outputs']['contact_out']),
            'contact_336_out': _to_numpy(contact_results['raw_outputs']['contact_336_out']),
            'contact_84_out': _to_numpy(contact_results['raw_outputs']['contact_84_out']),
            'contact_joint_out': _to_numpy(contact_results['raw_outputs']['contact_joint_out']),
        }
